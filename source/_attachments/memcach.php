...
define('ADMIN_USERNAME', 'james');  // Admin Username
define('ADMIN_PASSWORD', '123123');   // Admin Password
define('DATE_FORMAT', 'Y/m/d H:i:s');
define('GRAPH_SIZE', 200);
define('MAX_ITEM_DUMP', 50);

$MEMCACHE_SERVERS[] = '10.0.88.1:11211'; // add more as an array
...
function getMenu() {
    ...
    if ($_GET['op'] != 4) {
        echo <<<EOB
    <li><a href="http://jusfeel.blog.163.com/blog/$PHP_SELF&op={$_GET['op']}">Refresh Data</a></li>
EOB;
    }
    else {
        echo <<<EOB
    <li><a href="http://jusfeel.blog.163.com/blog/$PHP_SELF&op=2}">Back</a></li>
EOB;
    }
    echo
...
if (isset($_GET['IMG'])) {
    ...
    function fill_arc($im, $centerX, $centerY, $diameter, $start, $end, $color1, $color2, $text = '', $placeindex = 0) {
        ...
        if (function_exists("imagefilledarc")) {
            ...
            imagefilledarc($im, $centerX + 1, $centerY + 1, $diameter, $diameter, $start, $end,
                           $color1, IMG_ARC_PIE);
            imagefilledarc($im, $centerX, $centerY, $diameter, $diameter, $start, $end, $color2,
                           IMG_ARC_PIE);
            imagefilledarc($im, $centerX, $centerY, $diameter, $diameter, $start, $end, $color1,
        ...
        if ($text) {
            if ($placeindex > 0) {
                ...
                imagestring($im, 4, $centerX + $r * cos($w) / 2, $centerY + $r * sin($w) / 2, $text,
                                                                                     $color1);
            }
        }
    ...
    switch ($_GET['IMG']) {
        ...
        case 2: // hit miss
            ...
            fill_box($image, 130, $size, 50, -max(4, ($total - $hits) * ($size - 21) / $total),
                                                  $col_black, $col_red,
...
EOB;
        ...
        if (!isset($_GET['singleout']) && count($MEMCACHE_SERVERS) > 1) {
            foreach ($MEMCACHE_SERVERS as $server) {
                echo ($i + 1) . '. <a href="http://jusfeel.blog.163.com/blog/' . $PHP_SELF . '&singleout=' . $i++ . '">' . $server . '</a><br/>';
            }
        }
        else {
        ...
        if (isset($_GET['singleout'])) {
            echo '<a href="http://jusfeel.blog.163.com/blog/' . $PHP_SELF . '">(all servers)</a><br/>';
        }
        echo "</td></tr>\n";
...
EOB;
        foreach ($MEMCACHE_SERVERS as $server) {
            ...
            echo '<tr class=tr-1><td class=td-1>' . $server . '</td><td><a href="http://jusfeel.blog.163.com/blog/' . $PHP_SELF . '&server=' . array_search($server,
...
EOB;
        ...
        '<tr>',
        '<td class=td-0><span class="green box">&nbsp;</span>Free: ', bsize($mem_avail) . sprintf(" (%.1f%%)",
                                                                                                  $mem_avail * 100 / $mem_size), "</td>\n",
        '<td class=td-1><span class="green box">&nbsp;</span>Hits: ', $hits . sprintf(" (%.1f%%)",
                                                                                      $hits * 100 / ($hits + $misses)), "</td>\n",
        '</tr>',
        '<tr>',
        '<td class=td-0><span class="red box">&nbsp;</span>Used: ', bsize($mem_used) . sprintf(" (%.1f%%)",
...
EOB;
            ...
            foreach ($entries as $slabId => $slab) {
                ...
                echo
                "<tr class=tr-$m>",
                "<td class=td-0><center>", '<a href="http://jusfeel.blog.163.com/blog/', $dumpUrl, '">', $slabId, '</a>', "</center></td>",
                ...
                if ((isset($_GET['dumpslab']) && $_GET['dumpslab'] == $slabId) && (isset($_GET['server']) && $_GET['server'] == array_search($server,
                    ...
                    foreach ($items['ITEM'] as $itemKey => $itemInfo) {
                        ...
                        echo '<a href="http://jusfeel.blog.163.com/blog/', $PHP_SELF, '&op=4&server=', (array_search($server,
...
EOB;
        ...
        '<td><a href="http://jusfeel.blog.163.com/blog/', $PHP_SELF, '&op=5&server=', (int) $_GET['server'], '&key=', base64_encode($theKey), "\">Delete</a></td>", "</tr>";
        echo <<<EOB
   </tbody></table>
   </div><hr/>
EOB;
